...
# version that uses langchain instead of embedchain
# use multiple memory types (vector and summarize): ConversationSummaryBufferMemory
class ExpertSolo2:
    def __init__(self, expert:ExpertModel, session_data:dict = None, vector_config:dict = None, session_id:str = None, language:str = "en", envs:dict = {}, public_url:str = None):
        ...
        # As an experienced PR professional, your objective is to represent the user's interests within the team. You have the ability to make one phone call to the user per session to gather necessary information. 
        # Your main goal in this session is to engage the user in a natural conversation to gather their preferences, using the questions provided.
        # You have extensive experience in customer service and are skilled in negotiation.
        # During a phone conversation with {self.session_data.get('user_name', 'someone')}, you aim to clarify the following preferences:
        ```
        {str(self.session_data.get('queries', {}))}
        ```
        # Your task for this session is to engage the user in a conversation that naturally elicits responses to all the above queries.
        # You may prompt the user for elaborations or clarifications as necessary, but avoid parroting the user's exact words. Focus on the essential queries.
        """
        if self.session_data.get('intro'):
            self.system_prompt += f"""# Your opening statement in the conversation was: '{self.session_data.get('intro')}'."""
...
# You must ensure all your responses are in Spanish, maintaining patience throughout the conversation, but always reply in Spanish.
# Throughout the conversation, maintain a patient and engaging demeanor, ensuring all communications are conducted in Spanish.
            """
